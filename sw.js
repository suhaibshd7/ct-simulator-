...
const CACHE = 'nexagen-ct-v3'; // bumped: v2→v3 adds local font file caching
const ASSETS = [
  './',
  './index.html',
  './manifest.json',
  ...
  e.waitUntil(
    caches.open(CACHE).then(cache =>
      cache.addAll(['./index.html', './manifest.json'])
        ...
        .then(() => cache.add('./fonts/ShareTechMono-Regular.ttf').catch(() => {}))
        .then(() => cache.add('./fonts/Rajdhani-Regular.ttf').catch(() => {}))
        .then(() => cache.add('./fonts/Rajdhani-Medium.ttf').catch(() => {}))
        .then(() => cache.add('./fonts/Rajdhani-SemiBold.ttf').catch(() => {}))
        .then(() => cache.add('./fonts/Rajdhani-Bold.ttf').catch(() => {}))
    ).then(() => self.skipWaiting())
  );
});

// Activate: delete any old caches (including the old cache-first v1)
